...
class RetrievalService:
    ...
    def search_knowledge(
        ...
        *,
        ...
        source_kind: Literal["document", "interview", "mixed"] | None = None,
        limit: int = 5,
    ) -> SearchSuccess:
        if len(query) > 1500:
        ...
        for item in items:
            ...
            try:
                source_text = " ".join(
                    str(segment["text"])
                    for segment in self.repository.read_segments(workspace_id, evidence_ids)
                )
            except Exception as exc:
                raise SearchFailure(
                    "知識の原文取得に失敗したため、検索結果を確定できません。"
                ) from exc
            searchable = " ".join(
                [
                    item.title,
                    ...
                    item.case_label or "",
                    *item.tags,
                    *(f.text for f in item.facts),
                    source_text,
                ]
            )
            item_grams = bigrams(normalize_text(searchable, self.aliases))
            overlap = (
                len(query_grams & item_grams) / max(len(query_grams), 1) if query_grams else 1.0
            )
            ...
            condition_matches = {
                ...
                in {ConditionScope.CASE_CONTEXT, ConditionScope.APPLICABILITY}
            }
            ...
            if not query_grams or score >= 0.12:
                hits.append(
                    SearchHit(
                        ...
                        source_kind=cast(
                            Literal["document", "interview", "mixed"], item.source_kind
                        ),
    ...
    def _equipment_from_query(self, normalized_query: str) -> str | None:
        matches = set(re.findall(r"(?:冷却器|ポンプ|貯槽)\d+", normalized_query))
        if len(matches) > 1:
